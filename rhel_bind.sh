...
case "$choice" in 
  y|Y ) echo "Please specify a new hostname"
	...
	hostname
	#write change to log:
	echo "hostname updated to" >> /tmp/rhel_bind.log; echo hostname >> /tmp/rhel_bind.log
	sleep 1
	BelayThat
		;;
  n|N ) echo "You have chosen to leave your hostname as it is"
		echo "hostname not changed" >> /tmp/rhel_bind.log
		sleep 2
		BelayThat
		;;
esac
...
Set_Time () {
	...
	systemctl start ntp
	date
	echo "time set to ntp server" >> /tmp/rhel_bind.log; date >> /tmp/rhel_bind.log
	sleep 2
}
##################################################################################
...
realm-manager () {
	...
	read -p "allow all domain-users (y allows all, n will prompt to specify users) (y/n)?" choice2
case "$choice2" in 
  y|Y ) echo "confirmed - all users in your domain will be able to login locally - press return to continue"
	read confirmation
	realm permit --all
	realm list
	sleep 5
	echo "changes can be made later with the command 'realm list' and 'realm permit -u <AD_username>'"
	sleep 3
		;;
  n|N ) echo "please enter the usernames of the users you wish to add in the following format:"
		echo "<ad_username1> <ad_username2> <ad_username3> ...."
		read access-users
...
MENU_LAUNCH () {
	clear
	echo "${INTRO_TEXT} 	Active Directory link 						${END}"
	echo "${NORMAL}  														${END}"
...
yum install -y samba-common samba-common-tools
...
echo "beginning permissions staging." 
sleep 2
clear
...
sleep 3
echo "Please restart your workstation and login as your AD credentialed user account"
sleep 3
